...
console.log(' Le Worker WhatsApp ZENO-IA a été réveillé et guette la file...');
...
const whatsappWorker = new Worker('whatsapp-queue', async (job) => {
    ...
    try {
        ...
        // Sécurité : Si aucun prompt n'est configuré en BDD, on utilise un prompt Zeno de secours
        const currentPrompt = systemPromptRecord 
            ? systemPromptRecord.content 
            : `Tu es l'assistant officiel de Zeno au Congo-Brazzaville. Sois professionnel et guide l'utilisateur vers la validation de son identité.`;
